...
const AdminPanel = () => {
    ...
    const handleBanUser = async (userId, username) => {
        ...
        try {
            const result = await db.banUser(userId, parseInt(duration), reason);
            if (result.success) {
                toast({
    ...
    const handleUpdateTokens = async (userId, username, currentTokens) => {
        const newAmount = prompt(`Tokens actuales de ${username}: ${currentTokens}\n\nNuevo valor:`, currentTokens);
        if (!newAmount) return;

        try {
            const result = await db.updateUser(userId, { tokens: parseFloat(newAmount) });
            if (result.success) {
                toast({
    ...
    const handleProcessWithdrawal = async (userId, withdrawalId, action, amount, userName) => {
        try {
            const result = await db.processWithdrawal(userId, withdrawalId, action);
            if (result.success) {
                toast({
