...
const ModeratorPanel = () => {
    ...
    const loadData = async () => {
        try {
            const matches = await db.getDisputedMatches();
            const allReports = await db.getAllReports();
            setDisputedMatches(matches || []);
            setReports((allReports || []).filter(r => !r.reviewed && r.status === 'pending'));
        } catch (error) {
    ...
    const handleResolveDispute = async (matchId, winnerId) => {
        ...
        try {
            const result = await db.resolveDispute(matchId, winnerId, currentUser.id, notes || '');
    ...
    const handleReviewReport = async (reportId, action) => {
        ...
        try {
            const result = await db.reviewReport(reportId, action, currentUser.id, notes || '');
            
            if (result.success) {
                toast({
                    title: action === 'resolved' ? "Reporte Aprobado" : "Reporte Rechazado",
                    description: "El reporte ha sido procesado",
                    className: action === 'resolved' ? "bg-green-950 border-green-800 text-white" : "bg-blue-950 border-blue-800 text-white"
                });
